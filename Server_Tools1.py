...
async def data_visualization(
    ...
    """Creates data visualizations from database tables. Use ONLY for chart/graph requests, not general data queries.Creates data visualizations from database tables. Supports bar, line, pie, scatter, and multi-chart dashboards."""
    if not any(keyword in chart_type.lower() for keyword in ["chart", "graph", "plot", "visual", "dashboard", "bar", "line", "pie", "scatter"]):
        return {
        "error": "Please specify a valid chart type (bar, line, pie, scatter, multi)",
        "suggestion": "For data queries without visualization, use the appropriate CRUD tool directly"
    }
