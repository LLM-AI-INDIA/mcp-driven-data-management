...
def detect_visualization_type(query: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    Returns (chart_type, data_subject) tuple.
    """
...
def create_sales_visualizations(df, chart_type=None):
    ...
    # If specific chart type requested, create that chart
    if chart_type:
        if chart_type == "pie" and 'customer_name' in df.columns and 'total_price' in df.columns:
            customer_sales = df.groupby('customer_name')['total_price'].sum().reset_index()
            fig = create_visualization(customer_sales, "pie", "customer_name", "total_price", 
                                     "Sales Distribution by Customer")
            if fig:
                st.plotly_chart(fig, use_container_width=True, key=get_unique_viz_key('sales_customer_pie'))
        
        elif chart_type == "bar" and 'product_name' in df.columns and 'total_price' in df.columns:
            product_sales = df.groupby('product_name')['total_price'].sum().reset_index()
            fig = create_visualization(product_sales, "bar", "product_name", "total_price", 
                                     "Total Sales by Product", "total_price")
            if fig:
                st.plotly_chart(fig, use_container_width=True, key=get_unique_viz_key('sales_product_bar'))
        
        elif chart_type == "line" and 'sale_date' in df.columns and 'total_price' in df.columns:
            try:
                df['sale_date'] = pd.to_datetime(df['sale_date'])
                daily_sales = df.groupby(df['sale_date'].dt.date)['total_price'].sum().reset_index()
                fig = create_visualization(daily_sales, "line", "sale_date", "total_price", 
                                         "Sales Over Time")
                if fig:
...
# Similarly update other visualization functions with the same pattern
def create_customer_visualizations(df, chart_type=None):
    """Create visualizations for customer data with optional specific chart type"""
    if df.empty:
        return
    
    if chart_type:
        if chart_type == "bar" and 'FirstName' in df.columns:
            name_counts = df['FirstName'].value_counts().reset_index()
            name_counts.columns = ['FirstName', 'Count']
            fig = create_visualization(name_counts, "bar", "FirstName", "Count", 
                                     "Customer Count by First Name", "Count")
            if fig:
                st.plotly_chart(fig, use_container_width=True, key=get_unique_viz_key('customer_names_bar'))
        
        elif chart_type == "pie" and 'Email' in df.columns:
            ...
            fig = create_visualization(email_status, "pie", "Has_Email", "Count", 
                                     "Email Availability")
            if fig:
