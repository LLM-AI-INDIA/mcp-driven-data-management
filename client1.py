...
def render_visualization_section(viz_result, user_query):
    ...
    if not viz_result or viz_result.get("error"):
        error_msg = viz_result.get("error", "Unknown error occurred during visualization")
        st.error(f"❌ Visualization Error: {error_msg}")
    
        # Provide suggestions based on error type
        if "No data" in error_msg:
            st.info("💡 Try querying the data first with a command like 'show sales' or 'list products'")
        elif "field" in error_msg.lower():
            st.info("💡 Try specifying what to visualize, like 'bar chart of sales by product'")
        return
...
def create_chart_config_from_data(data, chart_type, data_source, x_field=None, y_field=None, aggregate=None):
    ...
    if data_source == "sales":
        x_field = x_field or "customer_name"
...
def parse_user_query(query: str, available_tools: dict) -> dict:
    ...
    non_visualization_keywords = [
    'list', 'show', 'display', 'get', 'find', 'search', 
    'create', 'add', 'update', 'change', 'modify', 'delete', 'remove',
    'count', 'how many'
]

    # Check if query contains both visualization and non-visualization keywords
    has_visualization_keywords = any(keyword in query.lower() for keyword in visualization_keywords)
    has_non_visualization_keywords = any(keyword in query.lower() for keyword in non_visualization_keywords)

    if has_visualization_keywords and has_non_visualization_keywords:
        is_visualization_request = False

    non_viz_patterns = [
        r'show (me )?(all )?(the )?records',
        r'list (all )?(the )?',
        r'display (all )?(the )?',
        r'get (all )?(the )?',
        r'find (all )?(the )?',
    ...
    for pattern in non_viz_patterns:
